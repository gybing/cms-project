...
(function($){
	...
	$.extend($.fn, {
		...
		setVal:function(value){//自定义方法，重新加载combox
			var $this = $(this);
			var id=$this.attr("id");
			$this.val(value);
			var ul=$("#op_combox_"+id);
			var box=$("#combox_"+ id);
			var ref = $this.attr("ref");
			var refUrl = $this.attr("refUrl") || "";
			ul.empty();
			$("option", $this).each(function(){
				var option = $(this);
				ul.append("<li><a class=\""+ (value==option[0].value?"selected":"") +"\" href=\"#\" value=\"" + option[0].value + "\">" + option[0].text + "</a></li>");
			});
			var label = $("option[value=" + value + "]",$this).text();
			var a=$("#combox_"+id).find(">a");
			a.val(value);
			a.text(label);
			if (ref && refUrl) {
				$this.unbind("change", _onchange).bind("change", {ref:ref, refUrl:refUrl, $this:$this}, _onchange);
			}
			$("#op_"+box.attr("id")).find(">li").comboxOption($(">a",box), box);
		}
	});
})(jQuery);
