		...
		return this.each(function(){
			...
			$trs.hoverClass().each(function(){
				...
				$tr.dblclick(function(){  
					if($tr.attr('target')=='DBLCLICK_USER_LIST_ID'){ // 用户详情页面 table 双击事件
						openNavTabUserDetail($tr.attr('rel'));	// 调用 用户列表页面打开用户详情方法
					}else if($tr.attr('target')=='DBLCLICK_USER_AUTH_ID'){ // 个人认证 
						openNavTabAddUserAudit($tr.attr('rel'));	
					}else if($tr.attr('target')=='DBLCLICK_COMPANY_AUTH_ID'){ // 企业认证
						openNavTabAddCompanyAudit($tr.attr('rel'));
					}else if($tr.attr('target')=='DBLCLICK_DRIVER_AUTH_ID'){ // 司机认证 
						openNavTabAddDriverAudit($tr.attr('rel'));	
					}else if($tr.attr('target')=='DBLCLICK_TRUCK_AUTH_ID'){ // 车辆认证
						openNavTabAddTruckAudit($tr.attr('rel'));	
					}else{
						// 其他页面不处理
					}
				});
