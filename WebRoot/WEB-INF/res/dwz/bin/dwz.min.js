	...
	$.fn.extend({
		...
		layoutH: function($refBox){
			return this.each(function(){
				...
				var a=parseInt(paddingTop,10)+parseInt(paddingBottom,10);
				var iH = iRefH - iLayoutH > 50 ? (iRefH-$this.offset().top+header-a) : 50;
				if($refBox.attr("class").indexOf("dialogContent")!=-1){
...
//					$label.css(altBoxCss(1));
					$label.css(altBoxCss(0.3));//alt初始样式
					if ($this.val().trim() && $this.val().trim()!=$this.attr("alt")) {
						$label.hide();
					}
					
				}
				
				//鼠标获取焦点、失去焦点事件绑定-alt样式变化
				$this.focus(function(){
					getAltBox().css(altBoxCss(0.3));
				}).blur(function(){
					if ($(this).val().trim() && $(this).val().trim()!=$this.attr("alt")) {
						getAltBox().hide().css("opacity",1);
					} else { 
						$(this).val(""); 
						getAltBox().show().css("opacity",0.3);
					}
				}).keydown(function(){
					getAltBox().hide();
				});
...
//					$.pdialog.switchDialog(dialog);
		...
		*//**
		...
		//正确的pdialog对象 （从dwz.dialog.js文件复制过来的）
		function($){
			var currentDialogId ="";
			$.pdialog = {
				_op:{height:300, width:580, minH:40, minW:50, total:20, max:false, mask:false, resizable:true, drawable:true, maxable:true,minable:true,fresh:true},
	...
	$.fn.extend({
		...
		itemDetail: function(){
			return this.each(function(){
				...
				var inpType = $table.attr('inpType');//可扩展其他状态，如：readonly、disabled
				if (addButTxt) {
			...
			function tdHtml(field){
				...
				switch(field.type){
					...
					case 'date':
						...
						break;
					case 'focus':
						html = '<input type="text" onfocus="customFun(this)" name="'
						     + field.name
						     + '" value="'
						     + field.defaultVal
						     + '" size="'
						...
						break;
					case 'blur':
						html = '<input type="text" onblur="customFunBlur(this)" name="'
							+ field.name
							+ '" value="'
							+ field.defaultVal
							+ '" size="'
	...
	$.extend($.fn, {
		combox:function(){
			return this.each(function(i){
				...
				var url=$this.attr("ajax");//异步请求url
				var value=$this.attr("defVal");//默认值
				var ajaxCallBack=$this.attr("ajaxCallBack")||null;//ajax回调方法
				var isSearch=$this.attr("isSearch")||"false";//是否显示搜索框，true|false
				var s2param={};
				if(isSearch=="false"){
					s2param.minimumResultsForSearch="Infinity";
				}
				s2param.width = $this.attr("width")||"150px";//宽度
				if(url && url!=""){
//					s2param.ajax={
//					    url: url,
//					    dataType: 'json',
...
//					}
//					s2param.cache=true;
//					var $select=$this.select2(s2param);
//					if(value){
//						$select.val(value).trigger("change");
//					}
					if(ajaxCallBack!=null){
						try{
							ajaxCallBack=eval(ajaxCallBack);
					...
					$.ajax({
						...
						success:ajaxCallBack||function(data){
							var json = data.resultObj;
							s2param.data=$.map(json, function(obj){
								obj.id = obj.id || obj.ID;
								obj.text = obj.text || obj.TEXT;
								return obj;
							});
