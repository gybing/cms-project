...
(function ($) {
    ...
    function setValue($input, keywords, opts){
        ...
        if (opts && opts.multiWord) {//多关键字支持，只设置 val
            ...
            $input.attr("value", id).focus().val(key);//+++++++++lpr++++++++++
        }
    ...
    */
    ...
    function refreshDropMenu($input, data, opts) {
        ...
        for (i = 0; i < len; i++) {
            ...
            tr = '<tr data-index="' + (data.value[i].__index || i) + '" value="' + idValue + '"" data-id="' + idValue + '" data-key="' + keyValue +'">' + tr + '</tr>';
                        ...
                        if (event.keyCode === options.keyDown) { //如果按的是向下方向键
                            if (currentList.length === 0) {
                                ...
                                if (options.autoSelect) {
                                    ...
                                    $(this).val($(this).attr('alt')).attr("value", "");  //+++++++lpr+++++++
                                }
                            } else {
                                unHoverAll($dropdownMenu, options);
                                //将原先选中列的下一列选中
