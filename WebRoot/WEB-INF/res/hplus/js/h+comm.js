 ...
 * @param aaSortParam设置列排序
 * @param idName设置选中行时回填值的字段名
 * @param isShowRowDetails 是否需要展示行的附加信息
 * @returns
 */
function initTableAutoHeight(id,url,param,colsParam,aoColumnParam,aaSortParam,idName,isShowRowDetails){
	isShowRowDetails = isShowRowDetails == true?isShowRowDetails:false; // 是否展示行的附加信息
	var pageHeight=$("#content-main",window.parent.document).height();
	height=pageHeight-$("#"+id).offset().top-50;
	var table=$("#"+id).DataTable({
		"searching": true, //搜索
...
//			"targets": aoColumnParam,   //指定的列 
		...
		"createdRow": function ( row, data, index ) {  //创建行信息 行被创建时回调函数
			if(isShowRowDetails){ // 展示行的附加信息按钮
...
function initCustomTable(id,url,colsParam,idName){
	var pageHeight=$("#content-main",window.parent.document).height();
	height=pageHeight-$("#"+id).offset().top-50;
	var table=$("#"+id).DataTable({
		"searching": true, //搜索
		...
		"ajax": {
			"url": url,  //Ajax请求地址
			"data":function(d){
				var data =  $.extend({}, d,null, {"numPerPage": d.length });
				return data;
			}, //Ajax请求时发送额外的数据(条件)，格式： { "name": "more_data", "value": "my_value" }
			"type": "POST"
		},
		"columns":null, //设置列属性条件   【列表信息】
		"ordering":false, //禁用表头排序功能
		"columnDefs": [
		               { //设置哪些列不进行排序
//			"orderable": false,//禁用排序 
