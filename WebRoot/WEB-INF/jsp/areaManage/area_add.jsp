...
<body class="">
		...
		<div class="col-sm-12">
			<div class="ibox float-e-margins">
				<div class="ibox-content widget-content nopadding">
					<form id="areaAddForm" method="post" action="#"
						data-id="${param.pdataId}" callback="searchForm()"
						class="form-horizontal">
						<input id="tag" name="tag" type="hidden"
							value="${sessionScope.SESSION_USER_LOGIN_INFO.userInfo.TAG }" />
						<div>
						<table
							...
							<tr>
								...
								<td colspan="2"><input type="hidden" name="province"
									...
									<table>
										<tr>
											<td><select id="area_add_addr_prov"
												name="area_add_addr_prov" style="width: 67px">
													<option value="">请选择</option>
											</select></td>
											<td style="padding-left: 15px;"><select
												id="area_add_addr_city" name="area_add_addr_city"
												style="width: 68px">
													<option value="">请选择</option>
											</select></td>
										</tr>
	...
	<div class="modal inmodal fade" id="mapModal" tabindex="-1"
		role="dialog" aria-hidden="true">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-body">
					<input type="hidden" id="mapSelectLng" name="mapSelectLng" /> <input
						type="hidden" id="mapSelectLat" name="mapSelectLat" />
					<div id="areaSelectMap" style="width: 100%;height:250px;"></div>
					手动填写地址:<input type="text" id="writeAddr" name="writeAddr"
						style="width: 200px" /><span style="color: red">输入完成请按回车，然后在地图上点击鼠标。</span>
				</div>
	...
	<script type="text/javascript">
		...
		function submitAddAreaForm() {
			var prov = $("#area_add_addr_prov option:selected").text();
			var city = $("#area_add_addr_city option:selected").text();
			prov = prov == '请选择' ? "" : prov;
			city = city == '请选择' ? "" : city;
			$("#province").val(prov);
			$("#city").val(city);
		...
		function initArea(id, tag) {
			...
			$("#" + id + suffix_prov).attr("refUrl",
					"${ctxPath }/topic/ajax/queryArea?PARENT_CODE={value}");
			$("#" + id + suffix_city).attr("ref", id + suffix_county);
			$("#" + id + suffix_city).attr("refUrl",
					"${ctxPath }/topic/ajax/queryArea?PARENT_CODE={value}");
	...
	<script type="text/javascript"
		src="http://api.map.baidu.com/api?v=2.0&ak=vaOdcXMsIjnesIldud46GOtT"></script>
	<script type="text/javascript">
		var marker;
		var map;
