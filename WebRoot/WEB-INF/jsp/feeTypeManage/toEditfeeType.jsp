...
<body class="gray-bg">
	<div class="wrapper wrapper-content animated fadeInRight">
		<form id="fee_type_edit_form" method="post" action="#" data-id="" callback="" class="form-horizontal">
		<input type="hidden" name="f_id" value="${param.fee_id }" /> <!-- 费用类型id -->
			<div class="ibox">
				<div class="ibox-content">
					<div class="panel panel-default" style="margin-top: 5px;">
						<div class="panel-heading" style="padding: 5px">
							<span style="font-weight: bold;">费用类型信息11</span>
						</div>
						<div class="panel-body">
							<div class="form-group">
								<label class="col-sm-1 control-label">类型代码:</label>
								<div class="col-sm-2">
									<input id="fee_type_code" name="fee_type_code" value="${responseDataForm.resultObj.FEE_TYPE_CODE }" maxlength="14" class="required digits form-control" type="text"/>
									<span class="help-block m-b-none"><i class="fa fa-info-circle"></i> 必填项</span>
								</div>
								<label class="col-sm-2 control-label">类型名称:</label>
								<div class="col-sm-2">
		                    		<input id="type_name" name="type_name" value="${responseDataForm.resultObj.TYPE_NAME }" maxlength="20" type="text" class="required form-control" />
									<span class="help-block m-b-none"><i class="fa fa-info-circle"></i> 必填项</span>
								</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-3">
									<input id="pay_price" name="pay_price" value="${responseDataForm.resultObj.PAY_PRICE }" maxlength="14" class="required number form-control" type="text"/><span class="help-block m-b-none"><i class="fa fa-info-circle"></i> 必填项</span>
								</div>
...
$(function() {
	$s2.init($C("#pay_way"), {
		...
		defVal:'${responseDataForm.resultObj.PAY_WAY }'
	});
	$s2.init($C("#count_type"), {
		sysdict : $sysdict.COUNT_TYPE,
		defVal:'${responseDataForm.resultObj.COUNT_TYPE }'
	});
	$s2.init($C("#pay_type"), {
		sysdict : $sysdict.PAY_TYPE,
		defVal:'${responseDataForm.resultObj.PAY_TYPE }'
	});
	$s2.init($C("#pay_cycle"), {
		sysdict : $sysdict.PAY_CYCLE,
		defVal:'${responseDataForm.resultObj.PAY_CYCLE }'
	});
	$s2.init($C("#cycle_unit"), {
		sysdict : $sysdict.CYCLE_UNIT,
		defVal:'${responseDataForm.resultObj.CYCLE_UNIT }'
	});
	$s2.init($C("#pay_unit"), {
		sysdict : $sysdict.PAY_UNIT,
		defVal:'${responseDataForm.resultObj.PAY_UNIT }'
	});
	$s2.init($C("#unit"), {
		sysdict : $sysdict.UNIT,
		defVal:'${responseDataForm.resultObj.UNIT }'
	});
	// 表单提交验证
	$("#fee_type_edit_form").validate({
		submitHandler : function(form) {
			// 验证楼宇编号是否选择
			$.ajax({
				url : "${ctxPath }/topic/ajax/updateFeeType",
				dataType : "json",
				data : $(form).serialize(),
				type : "post",
