...
<body class="gray-bg">
	<div class="wrapper wrapper-content animated fadeInRight">
		<form id="room_edit_form" method="post" action="#" data-id="" callback="" class="form-horizontal">
		<input type="hidden" name="r_id" value="${param.r_id }" /> <!-- 房间id -->
			<div class="ibox">
...
$(function() {
	...
	$s2.init($C("#room_type"), {
		...
		defVal: '${responseDataForm.resultObj.ROOM_TYPE }',
	});
	// 房间状态下拉框
	$s2.init($C("#room_state"), {
		sysdict : $sysdict.ROOM_STATE,
		defVal: '${responseDataForm.resultObj.ROOM_STATE }',
	});
	// 设置楼层下拉框为 select2
	$("#room_floor").select2({
	...
	$("#building_no").change(function(){
		var _b_id = $(this).val()?$(this).val():'${responseDataForm.resultObj.BUILDING_NO }';
		$.ajax({
	...
	$("#room_edit_form").validate({
		submitHandler : function(form) {
			...
			$.ajax({
				url : "${ctxPath }/topic/ajax/updateRoomInfo",
				dataType : "json",
				data : $(form).serialize(),
				type : "post",
				success : function(json) {
