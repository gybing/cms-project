...
<body class="gray-bg">
	<div class="wrapper wrapper-content animated fadeInRight">
		<form id="edit_move_out_form" method="post" action="#" data-id="" callback="" class="form-horizontal">
			<input type="hidden" name="u_id" id="u_id" ${responseDataForm.resultObj.USER_ID }/> <!-- 住户id -->
			<input type="hidden" name="move_addr" id="move_addr"/> <!-- 住户迁出楼房信息 -->
			...
			<div class="ibox">
				<div class="ibox-content">
					...
					<div class="panel panel-default" style="margin-top: 5px;">
						...
						<div class="panel-body">
							<div class="form-group">
								...
								<div class="col-sm-2">
									<input type="text" class="form-control layer-date" id="move_out_time" name="move_out_time" value="${responseDataForm.resultObj.MOVE_OUT_TIEM }"  placeholder="YYYY-MM-DD" onclick="laydate({istime: true, format: 'YYYY-MM-DD'})" />
								</div>
							</div>
						</div>
					</div>
					<div class="panel panel-default" style="margin-top: 5px;">
						<div class="panel-heading" style="padding: 5px">
							<span style="font-weight: bold;">迁出原因</span>
						</div>
						<div class="panel-body">
							<div class="form-group">
								<div class="col-sm-2">
									<textarea id="move_out_reason" name="move_out_reason" rows="2" cols="123" style="resize: none;">${responseDataForm.resultObj.MOVE_OUT_REASON }</textarea>
								</div> 
							</div>
						</div>
					</div>
...
$(function(){
	...
	$("#edit_move_out_form").validate({
		submitHandler : function(form) {
			...
			$.ajax({
				url : "${ctxPath }/topic/ajax/userMoveOut",
				dataType : "json",
				data : $(form).serialize(),
				type : "post",
				success : function(json) {
...
function validate(){
	...
	if($("#move_in_time").val() == ""){
		layer.tips("请选择选择迁出时间！", '#move_in_time', {
			tips: 2
		});
		return false;
	}
